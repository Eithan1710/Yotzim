...
function shareWhatsApp(id){
  ...
  window.open('[https://api.whatsapp.com/send?text='+encodeURIComponent(plain),'_blank','noopener](https://api.whatsapp.com/send?text='+encodeURIComponent(plain),'_blank','noopener)');
}
...
function calendarUrl(ev){
  ...
  return '[https://calendar.google.com/calendar/render?'+p.toString](https://calendar.google.com/calendar/render?'+p.toString)();
}
...
function navHTML(ev){
  ...
  const opts=[
    {e:'🗺️',t:'Google Maps',href:'[https://www.google.com/maps/search/?api=1&query='+q](https://www.google.com/maps/search/?api=1&query='+q)},
    {e:'🚗',t:'Waze',href:'[https://waze.com/ul?q='+q+'&navigate=yes](https://waze.com/ul?q='+q+'&navigate=yes)'}
  ];
  if(IS_IOS)opts.push({e:'🍎',t:'Apple Maps',href:'[https://maps.apple.com/?q='+q](https://maps.apple.com/?q='+q)});
...
function detailHTML(ev){
  ...
  const isPast=ev.when+PAST_AFTER<Date.now(); '+hhmm(d):whenLabel(ev.when))+(trText(ev)?' '+trText(ev):''); canEdit="!isPast&&me&&(!ev.by||ev.by===me.id);" class="grab" const h="`<div" let sub="(isPast?ddmm(d)+'" ·></div><div class="dhead" style="--h:${k.h}"><span class="tile">${k.e}</span>
...
function submitForm(){
  ...
  if(when<Date.now()-30*60e3){toast('השעה LS.set('yotz.tr',form.transport); closeSheet(); const description="$('#f-descr').value.trim();" id="form.editId;" if(form.editId){ הזו כבר עברה');return}>store.updateEvent(id,{kind:form.kind,place,when,transport:form.transport,description}))
    ...
    (data.recommendations||[]).forEach((r, i)=>{
      ...
      html+=`<div class="ai-card">
        ...
        <div class="actrow" style="margin-top:14px">
          ...
          <a class="actbtn" href="[https://waze.com/ul?q=$](https://waze.com/ul?q=$){encodeURIComponent(r.name+' '+r.location)}&navigate=yes" target="_blank">📍 נווט</a>
        </div>
